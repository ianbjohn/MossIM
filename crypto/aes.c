...
void aes_decrypt(byte* ciphertext, byte* key) {
  ...
  //the documentation shows decryption in this order, which  is different from just the inverse steps in reverse order of encryption
  //Not entirely sure as to why this is, but I do know some of these substeps, as well as their inverses, are commutative
  //There is potentially room for optimization if key expansion is done differently in decryption, and steps can be done in the same order as encryption.
  //I'm not too worried for right now though.
  aes_addroundkey(state, roundkeys[10]);
